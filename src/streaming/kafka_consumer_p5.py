...
from streaming.data_validation.data_contract_sailors import (
    CONSUMED_FIELDNAMES,
...
def process_message(
    ...
    *,
    ...
    LOG.info(f"effective_tax_rate={enriched['effective_tax_rate']:.4f}")
    LOG.info(f"total={enriched['total']}")
    LOG.info(f"running_subtotal={stats.total + enriched['subtotal']:.2f}")

    # Update running statistics with the new total.
    stats.update(enriched["subtotal"])
    return enriched
...
def consume_messages(
    ...
    *,
    ...
    while consumed_count + skipped_count < MAX_MESSAGES:
        ...
        LOG.info(f"total=${enriched['subtotal']:.2f}")
...
def log_summary(
    ...
    if stats.count > 0:
        LOG.info(f"  Total subtotal:  ${stats.total:,.2f}")
        LOG.info(f"  Average subtotal: ${stats.mean:,.2f}")
        LOG.info(f"  Minimum subtotal: ${stats.minimum:,.2f}")
        LOG.info(f"  Maximum subtotal: ${stats.maximum:,.2f}")
